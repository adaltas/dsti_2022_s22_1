...
1. Using the HDFS CLI, take a look at the data used for this lab at `/education/dsti_big_data_2022_fall/resources/lab4/nyc_drivers/drivers.csv`

2. Copy the `nyc_drivers` folder to your user directory in HDFS:

   ```bash
   hdfs dfs -mkdir -p "/education/dsti_big_data_2022_fall/$USER/lab4"

   hdfs dfs -cp /education/dsti_big_data_2022_fall/resources/lab4/nyc_drivers "/education/dsti_big_data_2022_fall/$USER/lab4"
   ```
...
4. Create an external table targeting our data with this statement (to be completed, replace `YOUR_USERNAME`):
   ...
   CREATE EXTERNAL TABLE dsti_big_data_2022_fall.${hiveUsername}_nyc_drivers_ext (
     driver_id INT,
     -- COMPLETE HERE
   )
...
2. Check that your table was created using the HDFS CLI at `/warehouse/tablespace/managed/hive/dsti_big_data_2022_fall.db/${USER}_nyc_drivers` (should be empty)
...
3. Check what the data looks like in the managed table using the HDFS CLI at `/warehouse/tablespace/managed/hive/dsti_big_data_2022_fall.db/${username}_nyc_drivers`
