...
Methods to choose the partition: round-robin, based on key, etc.
...
- 0 to M-1 “followers” → replication

## Apache Kafka performance

- The performance is **not impacted by the volume** stored
- Allow **scale of processing** → increase consumer instances
- Keep **records order** → 1 consumer receives records from 1 partition in order
- **Multiple independent “customers”** → 1 oﬀset per consumer group
...
## Dataflow: is that streaming?

Example of a Dataflow tool: [Apache NiFi](https://nifi.apache.org/)

Differences with streaming:

- Complex routing/filtering/formating logic
- Data is **not replicated** between nodes
- No OLAP capability
- **Focus on safe delivery of all the data** rather than low-latency
- User Interface to visualize the Dataflows
